...
void main() {
  ...
  group('Fuentes de tasa y motor', () {
    ...
    test('setModuleRateSource: inválido borra → global', () {
      store.setModuleRateSource(
        RateModule.converter,
        Currency.ves,
        'ves-parallel',
      );
      expect(
        store.settings.rateSourcesByModule['converter:VES'],
        'ves-parallel',
      );
      store.setModuleRateSource(
        RateModule.converter,
      ...
      expect(
        store.settings.rateSourcesByModule.containsKey('converter:VES'),
        isFalse,
      );
    });
  ...
  group('Productos (§2.2)', () {
    ...
    test(
      'addRecord estampa metSince al quedar bajo meta y limpia al subir',
      () {
        final p = store.addProduct(
          Product(
            id: '',
    ...
    test(
      'importProducts: barcode manda, nombre sin barcode no bloquea otro',
      () {
  ...
  group('Persistencia y resetAll (§2.5)', () {
    test(
      'round-trip Hive: los datos sobreviven una nueva hidratación',
      () async {
        store.addToCart(
          const CartItem(
            id: '',
    ...
    test('resetAll conserva SOLO el tablero vivo (§2.5 v15)', () {
      store.setRateBoard(
        RateBoard(
          sources: {'ves-bcv': RateEntry(rate: 40, updatedAt: DateTime(2026))},
        ),
      );
      store.addToCart(
        const CartItem(
  ...
  group('Snapshots · snapshotSeries (§4)', () {
    test('recorta a los últimos [days] días', () {
      ...
      final store2 = <SnapshotPoint>[
        SnapshotPoint(
          sourceId: 'ves-bcv',
          day: SnapshotPoint.dayKey(now.subtract(const Duration(days: 200))),
          rate: 1,
        ),
        SnapshotPoint(
          sourceId: 'ves-bcv',
          day: SnapshotPoint.dayKey(now.subtract(const Duration(days: 40))),
          rate: 2,
        ),
        SnapshotPoint(
          sourceId: 'ves-bcv',
          day: SnapshotPoint.dayKey(now.subtract(const Duration(days: 5))),
          rate: 3,
        ),
        SnapshotPoint(
          sourceId: 'ves-bcv',
          day: SnapshotPoint.dayKey(now),
          rate: 4,
        ),
      ];
  ...
  group('Sala · eventos remotos (guard de supresión)', () {
    test(
      'applyRemoteRoomEvent aplica item_add/update/remove/clear/replace',
      () {
  ...
  group('Metas de precio · checkProductTargets (17.7 · price_targets)', () {
    test(
      'anuncia el cruce fresco una vez, fija metSince y rearma al subir',
      () async {
        ...
        expect(
          store.products.firstWhere((x) => x.id == p.id).metSince,
          isNotNull,
        );
