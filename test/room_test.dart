...
class FakeLink implements RoomLink {
  ...
  void inject(String type, Map<String, dynamic> payload, [String fromId = '']) {
    _events.add(RoomEvent(type, {...payload, if (fromId.isNotEmpty) 'fromId': fromId}));
  }
  ...
  @override
  Future<void> sendTo(String id, String type, Map<String, dynamic> payload) async {
...
void main() {
  group('RoomProtocol · reglas del servidor (§6)', () {
    ...
    test('sanitizeItem: qty 1–999, price≥0, moneda 3 mayúsculas', () {
      final ok = RoomProtocol.sanitizeItem(
          {'id': 'i1', 'name': 'Café', 'quantity': 2, 'price': 3.5, 'currency': 'USD'},
          'me');
      expect(ok, isNotNull);
      expect(RoomProtocol.sanitizeItem(
          {'id': 'i1', 'name': 'X', 'quantity': 0, 'price': 1, 'currency': 'USD'}, 'me'),
          isNull);
      expect(
          RoomProtocol.sanitizeItem(
              {'id': 'i1', 'name': 'X', 'quantity': 1, 'price': 1, 'currency': 'usd'},
              'me'),
          isNull);
    });
    ...
    test('mensajes humanos de error (incluye BT)', () {
      ...
      expect(RoomProtocol.errorMessage('bt_timeout'), 'El otro teléfono no respondió a tiempo');
  ...
  group('RoomController · protocolo hello→welcome (enlace falso)', () {
    test('crear: el enlace arranca como ANFITRIÓN con su código real', () async {
      final link = FakeLink('cerca');
      final c = RoomController(_store(), linkFactory: (_) => link);
      final err = await c.create(name: 'Isa', roomName: 'Compras', isPublic: true);
      ...
      expect(link.hostStarted, isTrue, reason: 'el host NO puede arrancar como descubridor');
      expect(link.dialed, isFalse);
      expect(link.lastDialedCode, c.code, reason: 'el anuncio lleva el código real');
    ...
    test('unirse a una sala equivocada: error humano, sin sala fantasma', () async {
      final link = FakeLink('cerca');
      final c = RoomController(_store(), linkFactory: (_) => link);
    ...
    test('anfitrión: hello válido → welcome + members al resto', () async {
      ...
      expect(
          link.sent.any((m) =>
              m.type == 'room_error' && m.payload['reason'] == 'not_found'),
          isTrue);
      expect(c.visibleMembers.length, 2);
      await c.leave();
    });

    test('roles: viewer no toca la lista; your_role cambia en caliente', () async {
      final link = FakeLink('cerca');
      final c = RoomController(_store(), linkFactory: (_) => link);
      final future = c.join(name: 'Ana', code: 'ABC234');
      await Future<void>.delayed(const Duration(milliseconds: 20));
    ...
    test('cierre del anfitrión: room_closed llega y el invitado se va', () async {
      final link = FakeLink('cerca');
      final c = RoomController(_store(), linkFactory: (_) => link);
      final future = c.join(name: 'Ana', code: 'ABC234');
      await Future<void>.delayed(const Duration(milliseconds: 20));
    ...
    test('gobierno: kick y cambio de rol del anfitrión', () async {
      ...
      expect(link.sent.any((m) => m.type == 'your_role' && m.to == 'g1'), isTrue);
      expect(
          link.sent.any((m) => m.type == 'role_change' && m.to == 'g2'), isTrue);
