...
class _ItemEditorState extends State<_ItemEditor> {
  ...
  void initState() {
    ...
    _nameCtrl = TextEditingController(text: it?.name ?? widget.presetName ?? '');
    _priceCtrl = TextEditingController(
        text: it == null ? '' : fmtMoney(it.price, CurrencyX.from(it.currency)));
    _barcodeCtrl = TextEditingController(text: it?.barcode ?? widget.presetBarcode ?? '');
    _sizeCtrl = TextEditingController(
        text: (it?.size != null && it!.size! > 0) ? fmtNum(it.size!, decimals: 2) : '');
  ...
  double get _size => _mode == _SizeMode.none ? 0 : (parseLocaleNum(_sizeCtrl.text) ?? 0);
  ...
  void _save() {
    ...
    final item = (base ?? CartItem(id: '', name: name, quantity: qty, price: _price, currency: _currency.code))
        .copyWith(
          name: name,
          quantity: qty,
          price: _price,
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        Row(children: [
          Expanded(
            child: Text(editing ? 'Editar ítem' : 'Agregar ítem',
                style: TextStyle(
        ...
        // Cantidad.
        Row(children: [
          Text('CANTIDAD', style: VeText.labelCaps(9.5, color: scheme.onSurfaceVariant)),
          const SizedBox(width: 10),
          InkWell(
            onTap: () => setState(() => _qtyCtrl.text = '${(_qty - 1).clamp(1, 999)}'),
            borderRadius: BorderRadius.circular(8),
            child: const Padding(padding: EdgeInsets.all(6), child: Icon(Icons.remove_circle_outline, size: 20)),
          ),
          SizedBox(
            width: 52,
            child: TextField(
          ...
          InkWell(
            onTap: () => setState(() => _qtyCtrl.text = '${(_qty + 1).clamp(1, 999)}'),
            borderRadius: BorderRadius.circular(8),
            child: const Padding(padding: EdgeInsets.all(6), child: Icon(Icons.add_circle_outline, size: 20)),
          ),
        ]),
        ...
        Text('PESO / TAMAÑO', style: VeText.labelCaps(9.5, color: scheme.onSurfaceVariant)),
        const SizedBox(height: 6),
        Wrap(spacing: 6, children: [
          ChipTag('Sin peso', selected: _mode == _SizeMode.none,
              onTap: () => setState(() => _mode = _SizeMode.none)),
          ChipTag('Peso (g · kg)', selected: _mode == _SizeMode.weight,
              onTap: () => setState(() { _mode = _SizeMode.weight; _unit = _unit == 'l' || _unit == 'ml' ? 'g' : _unit; })),
          ChipTag('Volumen (ml · L)', selected: _mode == _SizeMode.volume,
              onTap: () => setState(() { _mode = _SizeMode.volume; _unit = _unit == 'g' || _unit == 'kg' ? 'ml' : _unit; })),
        ]),
          ...
          Row(children: [
            Expanded(
              child: TextField(
                ...
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                onChanged: (_) => setState(() {}),
                decoration: InputDecoration(hintText: _mode == _SizeMode.weight ? 'Contenido (600)' : 'Contenido (500)'),
              ),
            ),
            const SizedBox(width: 8),
            Wrap(spacing: 6, children: [
              for (final u in (_mode == _SizeMode.weight ? const ['g', 'kg'] : const ['ml', 'l']))
                ChipTag(u, selected: _unit == u, onTap: () => setState(() => _unit = u)),
            ]),
          ]),
        ],
