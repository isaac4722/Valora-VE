        ...
        children: [
          ...
          if (parPoint == null)
            Padding(
              ...
              child: Text(
                'El paralelo no tiene registro ese día: la brecha necesita ambas tasas (>0) el mismo día.',
      ...
      children: [
        ...
        if (_searched && _rateB == null && _rateP == null)
          Padding(
            padding: const EdgeInsets.only(top: 4),
            child: Text(
              'Sin registro local para esa fecha: los snapshots empiezan el día que la app '
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ReadWindow(
                    semanticLabel:
                        'Inflación personal de la canasta ${fmtPct(b.pct)}',
                    child: Row(
                      children: [
                        Expanded(
                          child: Text(
                        ...
                        Icon(
                          b.pct >= 0 ? Icons.trending_up : Icons.trending_down,
                          size: 20,
                          color: b.pct >= 0 ? ink.neg : ink.pos,
                        ),
                      ],
                    ),
                  ),
                  const SizedBox(height: 8),
                  Text(
                    'Costo de la canasta: ${fmtUSD(b.oldCost)} → ${fmtUSD(b.newCost)} · desde ${fmtDate(b.fromAt)}',
                    style: TextStyle(
                      fontSize: 11.5,
                      color: scheme.onSurfaceVariant,
                    ),
          ...
          children: [
            Expanded(
              child: StatCard(
                ...
                sub: prod == null
                    ? 'sin ≥2 registros en el rango'
                    : 'promedio simple · ${prod.products} productos',
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        const SizedBox(height: 10),
        Text(
          'Tu canasta: los productos que vigilas cada mes. Con ella se calcula '
                        ...
                        trailing: Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () => store.updateBasketItem(
                                b.productId,
                                b.quantity - 1,
                              ),
                            ),
        ...
        if (sources.isEmpty)
          ...
          if (multi)
            Padding(
              padding: const EdgeInsets.only(top: 6),
              child: Text(
                'Multi-divisa: hasta 3 fuentes superpuestas; la última no se quita.',
                style: TextStyle(
                  fontSize: 10.5,
                  color: scheme.onSurfaceVariant,
                ),
                    ...
                    series: [
                      for (var k = 0; k < selected.length; k++)
                        LineSeries<SnapshotPoint, DateTime>(
                          ...
                          yValueMapper: (p, _) => p.rate,
                          name:
                              RateSource.of(selected[k])?.label ?? selected[k],
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'CALENDARIO DE DEVALUACIÓN · 6 MESES',
              style: VeText.labelCaps(10, color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 2),
            Text(
