...
List<String> sListOf(dynamic v) => v is List
    ? v.map((e) => sOf(e)).where((s) => s.isNotEmpty).toList()
    : const [];
...
class RateEntry {
  ...
  static RateEntry? tryParse(Map<String, dynamic> j) {
    ...
    return RateEntry(
      rate: r,
      updatedAt: dtOf(j['updatedAt']) ?? DateTime.now(),
    );
  }
}
...
class Product {
  ...
  PriceRecord? get latestRecord => records.isEmpty
      ? null
  ...
  Product copyWith({
    ...
    unavailableSince: clearUnavailable
        ? null
...
extension FinanceCategoryX on FinanceCategory {
  bool get isIncome =>
      this == FinanceCategory.salario || this == FinanceCategory.otrosIngresos;
  String get label => switch (this) {
...
class CartItem {
  ...
  final String?
  store; // tienda del ítem (multitienda v17.2; null = única de la compra)
  ...
  factory ShoppingTemplate.fromJson(Map<String, dynamic> j) => ShoppingTemplate(
    ...
    items: (j['items'] is List
        ? (j['items'] as List)
              ...
              .map(
                (e) =>
                    ShoppingTemplateItem.fromJson(Map<String, dynamic>.from(e)),
              )
              .toList()
        : const <ShoppingTemplateItem>[]),
  );
...
class Settings {
  ...
  final String
  sseUrl; // 17.7: SSE en vivo opcional de un despliegue web ('' = off)
  ...
  Settings copyWith({
    ...
    targetParallel: clearTargetParallel
        ? null
        : (targetParallel ?? this.targetParallel),
    ...
    salaryVariable: clearSalaryVariable
        ? null
        : (salaryVariable ?? this.salaryVariable),
  ...
  double? effectiveSalary() {
    switch (salaryMode) {
      ...
      case 'rango':
        ...
        return (lo + hi) / 2;
      default:
        return (salaryAmount != null && salaryAmount! > 0)
            ? salaryAmount
            : null;
    }
  }
  ...
  static Settings fromJson(Map<String, dynamic> j) {
    ...
    return Settings(
      ...
      spikeWatch:
          ((j['spikeWatch'] as List?) ?? const ['ves-bcv', 'ves-parallel'])
              .map((e) => sOf(e))
              .where((s) => RateSource.of(s) != null)
              .toList(),
      rateSourcesByModule: ((j['rateSourcesByModule'] as Map?) ?? const {}).map(
        (k, v) => MapEntry(sOf(k), sOf(v)),
      ),
      ...
      tickerSpeed: switch (iOf(j['tickerSpeed'], 120)) {
        90 => 90,
        180 => 180,
        _ => 120,
      },
...
class Budget {
  ...
  static Budget fromJson(Map<String, dynamic> j) =>
      Budget(amount: dOf(j['amount']), currency: sOf(j['currency'], 'VES'));
}
...
class AppData {
  ...
  static AppData fromJson(Map<String, dynamic> j) => AppData(
    ...
    board: j['board'] is Map
        ...
        : const RateBoard.empty(),
    rateSource: ((j['rateSource'] as Map?) ?? const {}).map(
      (k, v) => MapEntry(sOf(k).toUpperCase(), sOf(v)),
    ),
    manualRates: ((j['manualRates'] as Map?) ?? const {}).map(
      (k, v) => MapEntry(sOf(k), dOf(v)),
    ),
    converter: j['converter'] is Map
        ? ConverterState.fromJson(
            Map<String, dynamic>.from(j['converter'] as Map),
          )
    ...
    templates: (j['templates'] is List
        ? (j['templates'] as List)
              .whereType<Map>()
              .map(
                (e) => ShoppingTemplate.fromJson(Map<String, dynamic>.from(e)),
              )
              .toList()
        : const <ShoppingTemplate>[]),
...
const List<CatalogSeed> kCatalogVE = [
  ...
  CatalogSeed(
    'Antiacídico pastillas',
    barcode: '7590211000197',
    category: ProductCategory.farmacia,
    presentation: Presentation.pack,
    size: 20,
    sizeUnit: null,
