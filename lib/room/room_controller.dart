...
class RoomController extends ChangeNotifier {
  RoomController(this._store, {RoomLink Function(RoomMode)? linkFactory})
    : __givenFactory = linkFactory;

  final AppStore _store;
  late final RoomLink Function(RoomMode) _linkFactory =
      __givenFactory ?? _defaultLink;
  final RoomLink Function(RoomMode)? __givenFactory;
  ...
  RoomLink _defaultLink(RoomMode m) => switch (m) {
    RoomMode.cerca => NearbyLink(),
    RoomMode.wifi => LanLink(),
    RoomMode.bt => BtLink(),
    // Modo Servidor (v19 · devuelto por orden del dueño): el relay
    // socket.io del propio usuario.
    RoomMode.servidor => ServerLink(
      serverUrl: _serverUrl(_store),
      token: serverToken,
    ),
  };

  /// URL del servidor socket.io del usuario (vive en los prefs del store).
  static String _serverUrl(AppStore store) =>
      store.prefs?.getString('valorave.liveServerAddr') ?? '';

  String get serverAddress =>
      _store.prefs?.getString('valorave.liveServerAddr') ?? '';
  String get serverToken =>
      _store.prefs?.getString('valorave.liveServerToken') ?? '';

  /// Fija la URL del servidor (vacía = sin servidor). Tolerante sin prefs.
  void setServerAddress(String v) {
    _store.prefs?.setString('valorave.liveServerAddr', v.trim());
    notifyListeners();
  }

  void setServerToken(String v) {
    _store.prefs?.setString('valorave.liveServerToken', v.trim());
    notifyListeners();
  }

  /// Prueba el servidor del usuario (ServerLink.probe): null = OK.
  Future<String?> probeServer() async {
    final url = serverAddress.trim();
    if (url.isEmpty) return 'Pega la URL de tu servidor (host:puerto)';
    final link = ServerLink(serverUrl: url, token: serverToken);
    return link.probe();
  }
  ...
  Duration _dialTimeout() => switch (_mode) {
    ...
    RoomMode.servidor => const Duration(seconds: 12),
        ...
        if (_isHost) {
          if (ev.type == 'item_add') {
            final item = RoomProtocol.sanitizeItem(
              ev.payload['item'],
              fromId.isNotEmpty ? fromId : 'host',
            );
            if (item == null) {
              if (fromId.isNotEmpty) {
                unawaited(
                  _link?.sendTo(fromId, 'room_error', {'reason': 'bad_item'}),
                );
              }
              return;
            }
  ...
  void _onWelcome(Map<String, dynamic> p) {
    ...
    _suppress = true;
    _store.applyRemoteRoomEvent('list_replace', {
      'items': (p['items'] as List?) ?? const [],
    });
    _suppress = false;
    _status = RoomStatus.connected;
    ...
    _members
      ...
      ..addAll(
        ((raw as List?) ?? const []).whereType<Map>().map(
          (m) => RoomMember.fromMap(Map<String, dynamic>.from(m)),
        ),
      );
  }
  ...
  Future<void> startScan() async {
    ...
    notifyListeners();
    switch (_mode) {
      case RoomMode.servidor:
        // El relay no anuncia salas: se entra con el código de 6 letras.
        _scanning = false;
        notifyListeners();
        return;
      case RoomMode.cerca:
